...
按名称/代码关键词搜索，支持独立的行业和市场过滤。`q` 只模糊匹配 `name` / `symbol`，不会搜索 `industry`；查行业应传 `industry` 精确值。接口自动排除 B 股代码（200xxx/900xxx），但退市 A 股（如 605081 退市太和）仍可能返回，需结合 `st` 接口或名称中的「退市/ST」标识甄别。
...
> **服务边界**：ApocData 主覆盖 A 股（沪深主板/创业板/科创板），`stocks` 自动排除 B 股，但可能包含退市 A 股。公告流仍可能含 A+H 公司的 H 股披露（见 E2）。不提供美股数据。
