...
按名称/代码关键词搜索，支持行业和市场过滤。**注意：搜索不区分板块，B 股（如 201872 招港B）与退市股（如 605081 退市太和）也在 stocks 库内、可被 `q=` 搜出；默认不带 `q` 的列表因分页固定仅返回前段深市主板。调用方需自行按 symbol 过滤：保留 A 股段（沪市 600/601/603/605/688，深市 000/001/002/003/300/301），排除 B 股 200xxx/900xxx；注意退市股（如 605081 退市太和）代码常落在正常 A 股段内，需结合 `st` 接口或名称中的「退市/ST」标识额外甄别。**
...
> **服务边界**：ApocData **主**覆盖 A 股（沪深主板/创业板/科创板）。但库内仍残留个别 B 股（如 201872 招港B）、退市股（如 605081 退市太和）及 H 股公告（见 E2），调用方需按 symbol 过滤、勿依赖这些残留数据；不提供北交所、美股数据。
