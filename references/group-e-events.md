...
按股票查公告（标题、公告日期、类型、链接；可选 AI 摘要和 Markdown 完整正文），**支持区间/类型/关键字过滤**，默认 5 条、上限 30 条。`summary` 由异步摘要管道生成，可能为 `null`，调用方必须判空，不能把摘要缺失当作无公告。正文尚未解析或正文较短时，服务端仍保留公告元数据，不会因此静默删除整行。
...
> - **H 股记录不会被接口静默删除**：服务端保留上游公告原始行；“是否纳入 A 股分析”由调用方决定。需要排查混入记录时可用 `q=H股`，不要把某一条记录暂时缺失直接判断为产品规则清理。
